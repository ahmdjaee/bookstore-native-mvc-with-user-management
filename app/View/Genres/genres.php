...
<main id="container" class="d-flex flex-column align-items-center" style="margin-left: 250px; transition: 0.5s; height: 100%">
    ...
    <div class="card p-3 pt-2  text-center border-0 " style="width: 95%; height: min-content; <?php echo (!isset($model['error']))  ? 'top: 24px;' : 'top: 0px;' ?> box-shadow: 0 0 4px 0 var(--shadow);">
        <header class=" w-100 d-flex align-items-center justify-content-between py-3 ">
            <div class="p-1 rounded w-50 text-start" style="background-image: url(<?= BASE_URL . '/assets/bg-header-table.png' ?>); background-repeat: no-repeat; background-size: cover">
                <h5 class="co--primary d-inline-block bg-white py-2 px-3 m-0 rounded" style="font-weight: 600 ;">Genres</h5>
            </div>
        ...
        <div class="card-body p-0 " style="min-height: 75vh;">
            <table class="table">
                ...
                <tbody class="text-start">
                    <?php if (isset($model['genres'])) : ?>
                        <?php foreach ($model['genres'] as $key => $genre) : ?>
                            <tr valign="middle">
                                <td><?= $genre->id ?></td>
                                <td><?= $genre->name ?></td>
                                <td>
                                    <button type="button" id="bookId" onclick="showModal(<?= $genre->id ?>)" class="btn btn-sm"><i class="fas fa-pencil-alt co--primary"></i></button>
                                    <form action="/genres/<?= $genre->id ?>/delete" style="display:inline;">
